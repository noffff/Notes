...
- 常见概念
  ...
  - 以太网首部：14字节
  - 以太网尾部：4字节
  - IP源站选路: 指定包在传输过程中要经过的路由
    - 宽松源站选路：可以通过其他接口来经过指定的路由
    - 严格源站选路：不能通过其他接口来转发包，如果经过其他接口则触发 ICMP差错报文
  - 以太网帧：包含原地址和目的地址MAC,各6bit，2字节长度。链路层封装 尾部4字节以太网尾部
  - 网卡混杂模式：只接受一个帧的副本
  - 广播地址：最高位的最低位为1
```
...
- 发现的流程
```
两端协商MSS，如果一端没有则使用默认的536字节。
选定段大小后,将在该连接的所有被TCP发送的IP数据报上都打上DF标志，禁止分片。如果路由器需要对报文分片则会丢弃，发送一个**不能分片差错**
收到ICMP差错后，TCP就减少段大小并重传
```
...
- 第一层
  ...
  - 总长度(16 bit IP数据报的总长度):最大长度为65535，当数据报被分段时，该值随着改变,但一般使用不超过576。
- 第二层
...
ICMP 源站抑制报文:表示收到的数据速度大于能够处理的速度。收到该报文，发送数据方会调用**慢启动**。该慢启动门阈ssthresh没有改变
主机不可达和网络不可达 通常被忽略，这两个差错通常由于中间路由器被关闭导致选路协议在进行选路时触发的。
```
